...
if ($facult_id > 0 && $srok_sdachi > 0 && $year_on_click) {
  $query = "SELECT SUM(izd_plan.sdano) as sdano, COUNT(izd_plan.facult_id) as total, SUM(izd_plan.izd_list) as sum, COUNT(izd_plan.facult_id), (SUM(izd_plan.sdano) / COUNT(izd_plan.facult_id)*100) as proc FROM izd_plan WHERE facult_id = $facult_id AND srok_sdachi = $srok_sdachi AND izd_plan.year = $year_on_click $not_include";
} else if ($facult_id > 0 && $srok_sdachi == 0 && $year_on_click) {
  $query = "SELECT SUM(izd_plan.sdano) as sdano, COUNT(izd_plan.facult_id) as total, SUM(izd_plan.izd_list) as sum, (SUM(izd_plan.sdano) / COUNT(izd_plan.facult_id)*100) as proc FROM izd_plan WHERE facult_id = $facult_id AND izd_plan.year = $year_on_click $not_include";
} else if ($facult_id == 0 && $srok_sdachi > 0 && $year_on_click) {
  $query = "SELECT SUM(izd_plan.sdano) as sdano, COUNT(izd_plan.facult_id) as total, SUM(izd_plan.izd_list) as sum, (SUM(izd_plan.sdano) / COUNT(izd_plan.facult_id)*100) as proc FROM izd_plan WHERE srok_sdachi = $srok_sdachi AND izd_plan.year = $year_on_click $not_include";
} else
